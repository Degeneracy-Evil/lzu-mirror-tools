CREATE TABLE IF NOT EXISTS schema_migrations(version INTEGER PRIMARY KEY, applied_at_ms INTEGER NOT NULL) STRICT;
...
CREATE TABLE IF NOT EXISTS mirror_schedule_state(
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_active_token_hash ON node_credentials(token_hash) WHERE revoked_at_ms IS NULL;
...
CREATE UNIQUE INDEX IF NOT EXISTS one_active_run_per_mirror ON runs(mirror_name) WHERE state IN('pending','running');
CREATE INDEX IF NOT EXISTS idx_runs_mirror_created ON runs(mirror_name,created_at_ms DESC);
CREATE INDEX IF NOT EXISTS idx_runs_state_created ON runs(state,created_at_ms);
CREATE INDEX IF NOT EXISTS idx_runs_node_created ON runs(owner_node,created_at_ms DESC);
...
CREATE INDEX IF NOT EXISTS idx_nodes_last_seen ON nodes(last_seen_at_ms);
